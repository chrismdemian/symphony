...
/**
 * Build sentry-source into a SINGLE self-contained file.
 *
 ...
 * reads its Sentry auth token (NOT a DSN) + org + projects from
 * `<install-dir>/config.json` (Symphony's env allowlist strips every
 * `SYMPHONY_*` var — a plugin sources its own secrets, never Symphony's
 * keychain). No CJS deps, so no `createRequire` banner needed.
 */
