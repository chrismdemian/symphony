...
/**
 * Build linear-source into a SINGLE self-contained file.
 *
 ...
 * reads its Linear API key from `<install-dir>/config.json` (Symphony's env
 * allowlist strips every `SYMPHONY_*` var — a plugin sources its own secrets,
 * never Symphony's keychain). No CJS deps, so no `createRequire` banner needed.
 */
