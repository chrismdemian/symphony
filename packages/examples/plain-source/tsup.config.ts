...
/**
 * Build plain-source into a SINGLE self-contained file.
 *
 ...
 * reads its Plain API key from `<install-dir>/config.json` (Symphony's env
 * allowlist strips every `SYMPHONY_*` var — a plugin sources its own secrets,
 * never Symphony's keychain). No CJS deps (raw `fetch` GraphQL), so no
