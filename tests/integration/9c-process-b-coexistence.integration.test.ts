/**
 * Phase 9C.1 — Process-B coexistence: the in-tree Linear / Jira connectors must
 ...
 * linear/jira clauses executable + regression-locked.
 *
 ...
 * `jira.json` sidecar for Jira's siteUrl + email). `tests/setup.ts` sets
...
const SOURCES: ReadonlyArray<{
  ...
  { id: 'linear-source', source: 'linear', boot: { linear: { enabled: true } } },
  { id: 'jira-source', source: 'jira', boot: { jira: { enabled: true } } },
];

beforeEach(() => {
  tmp = mkdtempSync(path.join(os.tmpdir(), 'sym-9c1-pb-'));
...
function configureInTree(source: string): void {
  ...
  if (source === 'jira') {
    writeFileSync(
      path.join(integrationsDir, 'jira.json'),
      JSON.stringify({ siteUrl: 'https://acme.atlassian.net', email: 'you@acme.com' }),
      'utf8',
    );
  }
}
...
describe('9C.1 Process-B coexistence — in-tree linear/jira yield when the plugin is enabled', () => {
  it.each(SOURCES)(
