/**
 * Phase 9C.1 — REAL plugin subprocess integration for the Linear + Jira
 ...
 *     `sync_linear` / `sync_jira` (host-built) + the plain
...
beforeEach(() => {
  ...
  // The fixtures' open issue routes to project `ENG` (a Linear team key / Jira
  // project key) via NormalizedIssue.projectValue.
...
describe('9C.1 real plugin subprocess — linear + jira issue-source bridge', () => {
  it.each([
    { id: 'linear-source', source: 'linear', externalId: '9a1b2c3d-0001' },
    { id: 'jira-source', source: 'jira', externalId: 'ENG-1' },
