/**
 * Phase 9C.2 — REAL plugin subprocess integration for the GitLab + Forgejo
 * issue-source plugins.
 *
 * Uses build-free `.mjs` fixtures (canned data + a get_writeback_log observer)
 * driven through the production StdioClientTransport — the real port (REST I/O)
 * is unit-tested directly against the example packages. Here we prove the bridge
 * end-to-end:
 *   - the host hides the issue-source internal tools and registers
 *     `sync_gitlab` / `sync_forgejo` (host-built) + the plain
...
describe('9C.2 real plugin subprocess — gitlab + forgejo issue-source bridge', () => {
  it.each([
    { id: 'gitlab-source', source: 'gitlab', externalId: 'acme/widgets#1' },
    { id: 'forgejo-source', source: 'forgejo', externalId: 'acme/repo#1' },
