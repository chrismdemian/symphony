// Phase 9C.1 — real MCP stdio server fixture for the Jira ISSUE-SOURCE plugin.
//
...
// Jira port (REST I/O + JQL chain + ADF) is unit-tested directly against the
// example package.
...
const server = new McpServer(
  { name: 'jira-source-fixture', version: '1.0.0' },
  { capabilities: { tools: {} } },
);
...
server.registerTool(
  'fetch_open_issues',
  {
    description: 'Canned Jira issues (issue-source contract).',
...
server.registerTool(
  ...
  async ({ externalId, status }) => {
    ...
    return {
      ...
      structuredContent: { written: true, code: 'written', value: "commented + transitioned to 'Done'" },
    };
  },
);
