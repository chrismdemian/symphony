/**
 * Phase 9C.3 production scenario — see 9c3.md.
 *
 * Boots the real orchestrator server with the plugin host active + the real
 * plain-source AND sentry-source subprocesses (build-free fixtures), connects a
 * Maestro-equivalent MCP client, and drives BOTH issue-source bridges through
 * the FULL server wiring (coexistence discovery, the plugin-writeback-refs
 * fan-out, the host issue-source deps):
 *   - `sync_plain` / `sync_sentry` are served by the plugins; the in-tree
...
beforeEach(() => {
  tmp = mkdtempSync(path.join(os.tmpdir(), 'sym-9c3-scn-'));
  ...
  for (const id of ['plain-source', 'sentry-source']) {
...
describe('9C.3 scenario — plain + sentry issue-source plugins end-to-end', () => {
  it('serves sync_plain + sync_sentry, in-tree yields, ingests, and writes back on completion', async () => {
    ...
    handle = await startOrchestratorServer({
      ...
      plain: { enabled: true }, // in-tree activation — must YIELD to the plugin
      sentry: { enabled: true }, // in-tree activation — must YIELD to the plugin
      rpc: { enabled: false },
    });
    ...
    expect(names).toContain('sync_plain');
    expect(names).toContain('sync_sentry');
    expect(names).toContain('plain-source__get_writeback_log');
    expect(names).toContain('sentry-source__get_writeback_log');
    expect(names).not.toContain('plain-source__fetch_open_issues');
    expect(names).not.toContain('sentry-source__write_back_status');
    // Coexistence gate held — exactly one of each (no in-tree duplicate).
    expect(names.filter((n) => n === 'sync_plain')).toHaveLength(1);
    expect(names.filter((n) => n === 'sync_sentry')).toHaveLength(1);

    await syncAndComplete(client, 'plain', 'th-1');
    await syncAndComplete(client, 'sentry', 'backend#1');
  }, 40_000);
});
