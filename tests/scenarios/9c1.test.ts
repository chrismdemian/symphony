/**
 * Phase 9C.1 production scenario — see 9c1.md.
 *
 * Boots the real orchestrator server with the plugin host active + the real
 * linear-source AND jira-source subprocesses (build-free fixtures), connects a
 ...
 *   - `sync_linear` / `sync_jira` are served by the plugins; the in-tree
...
beforeEach(() => {
  tmp = mkdtempSync(path.join(os.tmpdir(), 'sym-9c1-scn-'));
  ...
  for (const id of ['linear-source', 'jira-source']) {
...
describe('9C.1 scenario — linear + jira issue-source plugins end-to-end', () => {
  it('serves sync_linear + sync_jira, in-tree yields, ingests, and writes back on completion', async () => {
    ...
    handle = await startOrchestratorServer({
      ...
      linear: { enabled: true }, // in-tree activation — must YIELD to the plugin
      jira: { enabled: true }, // in-tree activation — must YIELD to the plugin
      rpc: { enabled: false },
    });
    ...
    expect(names).toContain('sync_linear');
    expect(names).toContain('sync_jira');
    expect(names).toContain('linear-source__get_writeback_log');
    expect(names).toContain('jira-source__get_writeback_log');
    expect(names).not.toContain('linear-source__fetch_open_issues');
    expect(names).not.toContain('jira-source__write_back_status');
    // Coexistence gate held — exactly one of each (no in-tree duplicate).
    expect(names.filter((n) => n === 'sync_linear')).toHaveLength(1);
    expect(names.filter((n) => n === 'sync_jira')).toHaveLength(1);

    await syncAndComplete(client, 'linear', '9a1b2c3d-0001');
    await syncAndComplete(client, 'jira', 'ENG-1');
  }, 40_000);
});
